...
def draw_controls_rebinding(screen, background, selected_control_index=0, rebinding_active=False, rebinding_control=None):
    ...
    # Controls list (movement, turning, shooting only - no spells)
    ...
    for i, (display_name, control_key) in enumerate(controls_list):
        ...
        # Display control name and current key
    ...
    if rebinding_active:
        ...
        instruction_text = small_font.render(f"Press new key for {rebinding_control[0] if rebinding_control else 'selected control'} (ESC to cancel)", True, (255, 255, 100))
        ...
        instruction_text = small_font.render("Use UP/DOWN to select, ENTER to rebind, ESC to go back", True, (220, 220, 220))
    ...
    while running:
                ...
                if event.key == pygame.K_F11:
                    ...
                    if menu_level == "main":
                        ...
                        if rebinding_active:
                            ...
                            # Normal rebinding menu navigation

                            if event.key == pygame.K_UP:

                                selected_control_index = (selected_control_index - 1) % 8  # 8 controls total (no spells)

                            elif event.key == pygame.K_DOWN:

                                selected_control_index = (selected_control_index + 1) % 8

                            elif event.key in (pygame.K_RETURN, pygame.K_KP_ENTER):

                                # Start rebinding for selected control
                    ...
                    pass  # Spell processing moved to frame update section



        if game_state == STATE_MENU:
            pass  # No spell processing in menu

        # Process spells every frame when playing (not just during events)
        ...
        if game_state == STATE_MENU:
            ...
            if menu_level == "rebinding":
                ...
                draw_controls_rebinding(screen, menu_background, selected_control_index, rebinding_active, rebinding_control)
