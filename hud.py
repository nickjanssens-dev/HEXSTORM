...
def draw_fullscreen_button(screen):
    ...
    pygame.draw.lines(
        ...
        (255, 255, 255),
        False,
        [(rect.x + m, rect.y + m + l), (rect.x + m, rect.y + m), (rect.x + m + l, rect.y + m)],
        2
    )
    ...
    pygame.draw.lines(
        ...
        (255, 255, 255),
        False,
        [(rect.right - m - l, rect.y + m), (rect.right - m, rect.y + m), (rect.right - m, rect.y + m + l)],
        2
    )
    ...
    pygame.draw.lines(
        ...
        (255, 255, 255),
        False,
        [(rect.x + m, rect.bottom - m - l), (rect.x + m, rect.bottom - m), (rect.x + m + l, rect.bottom - m)],
        2
    )
    ...
    pygame.draw.lines(
        ...
        (255, 255, 255),
        False,
        [(rect.right - m - l, rect.bottom - m), (rect.right - m, rect.bottom - m), (rect.right - m, rect.bottom - m - l)],
        2
    )
...
def load_hud():
    ...
    return pygame.transform.smoothscale(raw, (settings.SCREEN_WIDTH, HUD_HEIGHT))
...
def draw_minimap(screen, player, enemies):
            ...
            if enemy.alive:
                ...
                if 0 <= ex < len(game_map[0]) and 0 <= ey < len(game_map):
                    pygame.draw.rect(
                        screen,
                        (255, 50, 50),
                        (map_x + ex * tile_size, map_y + ey * tile_size, tile_size, tile_size)
                    )
    ...
    if 0 <= px < len(game_map[0]) and 0 <= py < len(game_map):
        ...
        pygame.draw.line(
            screen,
            (50, 255, 50),
            (map_x + px * tile_size + tile_size // 2, map_y + py * tile_size + tile_size // 2),
            (dir_x, dir_y),
            2
        )

def draw_hud(screen, player, hud_image, wave=None, kills=None, game_over=False, enemies=None, game_mode="normal"):
    hud_x = 0
